...
module.exports = {
  search: function(query) {
    ...
    var search_url = settings.THEPIRATEBAY_URL + "/search/" + search_query + "/0/7/0";
    var count = 1;
    var deferred = Q.defer();
    var data_content = {};
    var torrent_content = [];
    var link_content = [];
    // var ua = 'Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36';
    // var options = {
    //   url: "https://thepiratebay.org" + "/search/" + search_query + "/0/7/0",
    //   headers: {
    //     'User-Agent': ua
    //   }
    // };
    request(search_url, function(err, response, body){
      if(!err && response.statusCode === 200){
        ...
        deferred.reject(chalk.red('[-] ') + "There was a problem loading thepiratebay.");
      }
